...
<script>
	$(function() {
		...
		$("#userSearch").keypress(function(e) {
			...
			$.ajax({
				...
				success : function(data) {
					...
					$.each(data, function(i) {
						...
						str += '<td class="searchResult" onclick="addBoardMember(this)">' + data[i] + '</td>';
						str += '</tr>';
		 ...
		 function addBoardMember(item){

			var txt = $(item).text();
			var bfk = $("#bfk").val();
			var member = document.createElement("input");
			member.type = "hidden";
			member.name = "withMember";
			member.value = txt;
			$("#addBoardMemberForm").append(member);
			if(confirm(txt+" 추가하시겠습니까?")){
				$("#addBoardMemberForm").submit();
			} else{
				history.go(0);
			}
		}
...
<div class="background8"></div>
	...
	<form action="/member/addBoardMember" method="post" id="addBoardMemberForm">
	<input type="hidden" name="n" value="${bFk }" id="bfk" />
