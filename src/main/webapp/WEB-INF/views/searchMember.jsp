...
function searchClick(clicked_id) {
   ...
   var creatememberspace = '<tr class="over">'
                     + '<td id="' + clicked_id + clicked_id + '" onClick="remover(this.id);">' 
                     + member + '</td>'+ '<input id = "'+ clicked_id + clicked_id + clicked_id + clicked_id +'" type="hidden" name="withmember" value="' +member+ '"/>' + '</tr>'
...
function remover(removed_id) {
   console.log('removed_id 는 : ' + removed_id);
   $("#"+removed_id).remove();
   $("#"+removed_id+removed_id).remove();
}
</script>
<body>
